...
void ExampleLayer::resetState()
{
	...
	m_grid_shader.reset(
		Zyklon::Shader::create("examples/Grid/src/Shaders/Grid.shader"));
	...
	m_grid_index_bfr.reset(Zyklon::IndexBuffer::create(
		grid_indices, sizeof(grid_indices) / sizeof(uint32_t)));
...
{
	// ZYKLON_TRACE("DELTA TIME, {0}, {1}ms", ts.getSeconds(),
	// ts.getMilliseconds()); if (Zyklon::Input::keyPressed(ZYKLON_KEY_TAB))
	...
	m_grid_shader->setUniform1f(
		"u_time", Zyklon::Application::get().getWindow().getTime());
	m_grid_shader->setUniform2f("u_params", uniform_float_parameters[0],
								uniform_float_parameters[1]);
...
void ExampleLayer::onImguiRender()
{
	...
	if (ImGui::Button("reset"))
		resetState();
	ImGui::SliderFloat("Grid Scale", &uniform_float_parameters[0], 0.0f,
					   100.0f);
	ImGui::SliderFloat("Grid Resolution", &uniform_float_parameters[1], 0.0f,
					   1.0f);
	ImGui::End();
}