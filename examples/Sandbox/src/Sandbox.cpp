...
void ExampleLayer::resetState()
{
	...
	m_cube_shader.reset(
		Zyklon::Shader::create("examples/Sandbox/src/Shaders/Cube.shader"));
	...
	// m_cube_index_bfr.reset(IndexBuffer::create(cube_indices,
...
void ExampleLayer::onUpdate(Zyklon::Timestep ts)
{
	...
	m_model =
		glm::rotate(m_model, (glm::radians(model_rotation_speed * sin(ts))),
	...
	for (int i = 0; i < 5; i++) {
		for (int y = 0; y < 5; y++) {
			...
			Zyklon::Renderer::submitVertex(m_cube_shader, m_cube_vertex_array,
										   36, transform);
		}
	}
