...
OpenGLTexture2D::OpenGLTexture2D(const TextureSpecification &p_specification)
	: m_specification(p_specification), m_width(m_specification.width),
	  m_height(m_specification.height)
{
	m_internal_format =
		Utils::image_format_to_internal_format(m_specification.format);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, m_internal_format, m_width, m_height, 0,
				 m_data_format, GL_UNSIGNED_BYTE, nullptr);
	...
	: m_path(p_filepath) // implicit conversion
{
	...
	if (data) {
		...
		ZYKLON_CORE_ASSERT(internal_format & data_format,
...
void OpenGLTexture2D::setData(void *p_data, uint32_t p_size)
{
	...
	ZYKLON_CORE_ASSERT(p_size == m_width * m_height * bpp,
					   "Data must be entire texture!");
	glBindTexture(GL_TEXTURE_2D, m_renderer_id);
	glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_width, m_height, m_data_format,
