...
bool AssetManager::loadModel(const std::filesystem::path &p_filepath)
{
	...
	const aiScene *ai_scene =
		m_assimp_importer->ReadFile(p_filepath, ASSIMP_LOAD_FLAGS);
	...
	for (uint64_t i = 0; i < ai_scene->mNumMaterials; ++i) {
		...
		if (ai_mat->GetTexture(aiTextureType_DIFFUSE, 0, &str) == AI_SUCCESS) {
			std::string texture_full_path = model_dir_path + "/" + str.C_Str();
			Ref<Texture2D> z_texture = loadTexture(texture_full_path);
			ZYKLON_INFO("Loading texture {0}", p_filepath);
			if (z_texture) {
	...
	m_model_asset_data_cache[p_filepath] = model_data;
	return true;
}

void AssetManager::processAssimpNode(
	ModelNodeData &p_out_node_data, aiNode *p_ai_node,
	const aiScene *p_ai_scene, const std::filesystem::path &p_model_dir_path)
{
										 ...
										 Ref<Scene> target_scene)
{
	...
	// Decompose the local_transform to set individual transform properties
	glm::vec3 scale;
	glm::quat rotation;
	...
	// attach the meshrenderer components based on the mesh indices
	for (uint32_t mesh_idx : node_data.mesh_indices) {
		if (mesh_idx < model_asset_data.meshes.size()) {
			...
			if (!material_to_use) {
				// ensure the default shader exists before putting on the
				// basiclitmaterial
				if (AssetManager::getInstance().getDefaultShader()) {
					...
					ZYKLON_CORE_WARN(
						...
						model_asset_data.filepath, node_data.name);
				}
				else {
			...
			if (mesh && material_to_use) {
				...
				ZYKLON_CORE_ERROR(
					...
					"Mesh or Material is (after fallback)",
					node_data.name);
			}
		}
	}

	// recursively instantiate children game objects
...
{
	// maybe there'll be some issue if we load the full path instead of
	// the filename
	if (m_textures.count(p_path)) {
		ZYKLON_CORE_INFO("Texture '{0}' already loaded and cached.", p_path);
		return m_textures.at(p_path); // use for const-correctness instead of []
	}

	Ref<Texture2D> texture = Texture2D::create(p_path);

	if (texture) {
		m_textures[p_path] = texture;
		ZYKLON_CORE_INFO("Successfully loaded and cached texture: {0}", p_path);
	}
	else {
		ZYKLON_CORE_ERROR("Failed to load texture: {0}", p_path);
	}
...
{
	// check if materials is in cache
	if (m_materials.count(p_name)) {
		ZYKLON_CORE_INFO("Material '{0}' already loaded", p_name.c_str());
		return m_materials.at(p_name);
	}

	Ref<Material> material = createRef<BasicLitMaterial>(p_shader, p_texture);
