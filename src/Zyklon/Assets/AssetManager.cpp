	...
	AssetManager::AssetManager()
	{
		...
		// the model
		ShaderLibrary::getInstance().load("ZyklonDefaultLit", "src/Zyklon/EngineAssets/Shaders/BasicLit.shader");
		...
		if (m_default_texture) {
			...
			ZYKLON_CORE_ERROR("AssetManager: Failed to create fallback white texture!");
		}
	}
	...
	bool AssetManager::loadModel(const std::filesystem::path &p_filepath)
	{
		...
		if (m_model_asset_data_cache.count(p_filepath.string())) {
			ZYKLON_CORE_INFO("Model '{0}' already loaded and cached.", p_filepath.string());
			return true;
		}
		...
		if (!ai_scene || ai_scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE ||
			!ai_scene->mRootNode) {
			ZYKLON_CORE_ERROR("ASSIMP: Failed to load model '{0}': {1}", p_filepath.string(),
		...
		for (uint64_t i = 0; i < ai_scene->mNumMaterials; ++i) {
			...
			ZYKLON_CORE_INFO("Processing material '{0}'", ai_mat->GetName().C_Str());
			...
			ZYKLON_CORE_INFO("Looking for diffuse texture in material '{0}'", material_name);
			aiString str;
			if (ai_mat->GetTexture(aiTextureType_DIFFUSE, 0, &str) == AI_SUCCESS) {
				std::filesystem::path texture_full_path = std::filesystem::path(model_dir_path) / str.C_Str();
				...
				ZYKLON_WARN("Material '{0}' has no diffuse texture. Using default white texture.", material_name);
			}
	...
	void AssetManager::processAssimpNode(
		ModelNodeData &p_out_node_data, aiNode *p_ai_node,
		const aiScene *p_ai_scene, const std::string &p_model_dir_path)
	{
	...
	for (uint32_t mesh_idx : node_data.mesh_indices) {
		if (mesh_idx < model_asset_data.meshes.size()) {
            ...
            // Proper material assignment: Use the material index from the Assimp mesh.
            // (Assumes you've stored aiMesh->mMaterialIndex somewhere, or can retrieve it).
            // For now, still simplified to cycle or use the first if no direct mapping.
            // If model_asset_data.materials were loaded in the order of ai_scene->mMaterials:
            // unsigned int assimp_material_idx = ai_scene->mMeshes[mesh_idx]->mMaterialIndex; // Need ai_scene access or stored info
            // if (assimp_material_idx < model_asset_data.materials.size()) { material_to_use = model_asset_data.materials[assimp_material_idx]; }
            
            // Current simple fallback:
			if (!model_asset_data.materials.empty()) {
				material_to_use = model_asset_data.materials[mesh_idx % model_asset_data.materials.size()];
			}
			
            // If no material found from model data, fallback to a default BasicLitMaterial
			if (!material_to_use) {
				if (AssetManager::getInstance().getDefaultShader()) {
					...
					ZYKLON_CORE_WARN(
						"Model '{0}' has no specific material for mesh '{1}', using default BasicLitMaterial.",
	...
	Ref<Texture2D> AssetManager::loadTexture(const std::filesystem::path &p_path)
	{
		...
		if (texture) {
			...
			ZYKLON_CORE_INFO("Successfully loaded and cached texture: {0}", p_path.string());
		}
		else {
			ZYKLON_CORE_WARN("Failed to load texture: {0}. Using fallback white texture.", p_path.string());
			texture = m_default_texture;
			if (texture) { // Ensure fallback itself was created
				m_textures[p_path.string()] = texture; // Cache the fallback for this path too
			} else {
				ZYKLON_CORE_ERROR("AssetManager: No valid texture (even fallback) for path: {0}", p_path.string());
			}
		}
											...
											const Ref<Texture2D> p_texture)
	{
		...
		if (p_texture) {
			...
			ZYKLON_CORE_WARN("No texture provided for material '{0}', using default white texture.", p_name);
			actual_texture = m_default_texture;
		}

		if (!actual_shader) {
			ZYKLON_CORE_ERROR("AssetManager: Cannot create material '{0}', no shader provided and default shader is null!", p_name);
			return nullptr;
		}
		Ref<Material> material = createRef<BasicLitMaterial>(actual_shader, actual_texture);
