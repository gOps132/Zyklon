...
// scene manager is a singleton class that manages the current scene, loading
// and saving scenes, and resetting the scene
class SceneManager {
public:
	...
	SceneManager &
	operator=(const SceneManager &) = delete; // prevent assignment
	...
	// TODO: custom API for saving and loading scenes, potentially using
	// serialization
	void resetScene();
...
private:
	// possibly add private members to manage the current scene, scene history,
	// etc. For example, a map of loaded scenes or a stack for undo/redo
	// functionality.
	Ref<Scene> m_current_scene; // current active scene
	std::unordered_map<std::string, Ref<Scene>> m_loadedScenes;
	// may want to keep track of scene history for undo/redo
	// functionality std::stack<Ref<Scene>> m_sceneHistory; // for undo/redo
	// functionality std::stack<Ref<Scene>> m_sceneRedoStack; // for redo
	// functionality
};
