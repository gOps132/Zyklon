...
#include <Zyklon/Renderer/Camera/Camera.h>
...
private:
	...
	std::vector<Ref<GameObject>>
		m_root_game_objects; // game objects without parents
	std::unordered_map<UUID, Ref<GameObject>>
		m_all_game_objects; // all game objects in the scene with mapped UUID S
};
