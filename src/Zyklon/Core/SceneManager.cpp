...
void SceneManager::saveScene(const std::string &p_scene_name)
{
	...
	if (it != m_loadedScenes.end()) {
		...
		m_loadedScenes[p_scene_name] = m_current_scene;
	}
}
...
// NOTE: This function returns the name of the current scene, or an empty string
// if no scene is set
...
void SceneManager::setCurrentScene(const Ref<Scene> &p_scene)
{
	...
	if (m_current_scene) {
		...
		if (m_current_scene->getActiveCamera()) {
			...
			ZYKLON_CORE_WARN("No active camera set in scene {0}",
							 m_current_scene->getName());
		}
	}
	else {
...
void SceneManager::clearScenes()
{
	...
	if (m_current_scene) {
		m_current_scene
			->clearAllGameObjects(); // clear all game objects in current scene
		m_current_scene->setActiveCamera(nullptr); // reset active camera
		m_current_scene = nullptr;				   // reset current scene
	}
}
...
void SceneManager::render()
{
	if (m_current_scene) {
		...
		ZYKLON_CORE_WARN("No current scene to render.");
	}
}
